...
export default class Hikes {
  ...
  addSubmitListener(id) {
    let that = this;
    document.getElementById(id).addEventListener("click", (event) => createComment(event.target.id, that.comments));
  }
  ...
  buildBackButton() {
    ...
    backButton.innerHTML = '&larr;  All Hikes';
...
function renderOneHikeFull(hike) {
  ...
  item.innerHTML = ` 
    ...
    <div class="details-row">
        ...
        <button id="${hike.name}">Submit</button> </div>
        </div>
    `;
...
function createComment(hike, comments) { 
  let newComment = {   
    name: hike,
    date: new Date(),
    content: document.getElementById(`${hike}-input`).value
  };
